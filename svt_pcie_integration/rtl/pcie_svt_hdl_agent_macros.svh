...
`define PCIE_SVT_DECLARE_HDL_AGENT_X4(instance_name, display_name, clkreq_signal, wake_signal, reset_signal, is_root, hierarchy) \
  ...
  svt_pcie_single_port_device_agent_hdl instance_name``_spd(              \
    instance_name``_if);                                                   \
  pcie_svt_serial_port_if #(4) instance_name``_serial();                   \
  defparam instance_name``_spd.SVT_PCIE_UI_PCIE_SPEC_VER =                \
    `SVT_PCIE_UI_PCIE_SPEC_VER_5_0;                                        \
  defparam instance_name``_spd.SVT_PCIE_UI_DISPLAY_NAME = display_name;    \
  defparam instance_name``_spd.SVT_PCIE_UI_PHY_INTERFACE_TYPE =           \
    `SVT_PCIE_UI_PHY_INTERFACE_TYPE_SERDES;                                \
  defparam instance_name``_spd.SVT_PCIE_UI_TRANSMIT_BIT_CLOCK_MODE = 1'b1; \
  defparam instance_name``_spd.SVT_PCIE_UI_ENABLE_CFG_BLOCK = 1'b1;        \
  defparam instance_name``_spd.SVT_PCIE_UI_CONNECT_ACTIVE_VIP = 1'b1;      \
  defparam instance_name``_spd.SVT_PCIE_UI_NUM_PHYSICAL_LANES = 4;         \
  defparam instance_name``_spd.SVT_PCIE_UI_DEVICE_IS_ROOT = is_root;       \
  defparam instance_name``_spd.SVT_PCIE_UI_HIERARCHY_NUMBER = hierarchy;  \
...
`define PCIE_SVT_DECLARE_HDL_AGENT_X8(instance_name, display_name, clkreq_signal, wake_signal, reset_signal, is_root, hierarchy) \
  ...
  defparam instance_name``_spd.SVT_PCIE_UI_ENABLE_CFG_BLOCK = 1'b1;        \
  defparam instance_name``_spd.SVT_PCIE_UI_CONNECT_ACTIVE_VIP = 1'b1;      \
  defparam instance_name``_spd.SVT_PCIE_UI_NUM_PHYSICAL_LANES = 8;         \
  defparam instance_name``_spd.SVT_PCIE_UI_DEVICE_IS_ROOT = is_root;       \
  defparam instance_name``_spd.SVT_PCIE_UI_HIERARCHY_NUMBER = hierarchy;  \
...
`define PCIE_SVT_DECLARE_HDL_AGENT_X16(instance_name, display_name, clkreq_signal, wake_signal, reset_signal, is_root, hierarchy) \
  ...
  defparam instance_name``_spd.SVT_PCIE_UI_ENABLE_CFG_BLOCK = 1'b1;        \
  defparam instance_name``_spd.SVT_PCIE_UI_CONNECT_ACTIVE_VIP = 1'b1;      \
  defparam instance_name``_spd.SVT_PCIE_UI_NUM_PHYSICAL_LANES = 16;        \
  defparam instance_name``_spd.SVT_PCIE_UI_DEVICE_IS_ROOT = is_root;       \
  defparam instance_name``_spd.SVT_PCIE_UI_HIERARCHY_NUMBER = hierarchy;  \
